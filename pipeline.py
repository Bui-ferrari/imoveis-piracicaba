...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .badge-new{position:absolute;top:10px;right:10px;z-index:2;background:#f59e0b;color:#111;
             font-size:11px;font-weight:800;letter-spacing:.03em;padding:4px 9px;border-radius:20px;
             box-shadow:0 2px 6px rgba(0,0,0,.35);}
  .entry-date{font-size:11px;color:var(--muted);font-weight:600;}
...
function render(){
  ...
  filtered.forEach(item=>{
    ...
    const isNovo = item.dias_na_lista === 1;
    card.innerHTML = `
      ${isNovo ? '<span class="badge-new">🆕 NOVO HOJE</span>' : ''}
      ${thumb}
      <div class="body">
        ...
        <div class="days"><span class="entry-date">📅 Entrou na lista em ${item.first_seen}</span> · há ${item.dias_na_lista} dia(s) · visto por último em ${item.last_seen}</div>
      </div>
      <div class="actions">
